...
function classify(literal) {
  ...
  // Translucent values are shadows, scrims, or overlay copy, judged by lightness.
  if (c.a < 0.95) {
    if (l > 90) return { fg: '--wj-on-brand-muted', bg: '--wj-on-brand-muted', why: `white alpha ${c.a}` };
    // Navy and brand blue share a hue, so lightness is what separates a blue
    // focus glow from a navy shadow. rgba(20,26,60,.06) is --wj-shadow-sm.
    if (s > 40 && h > 200 && h < 260 && l > 45) {
      return { fg: '--wj-focus', bg: '--wj-focus', why: `blue alpha ${c.a}` };
    }
  ...
  // Everything left is on the blue/navy neutral spine, decided by lightness.
  ...
  if (l >= 60 && s > 45) return { fg: '--wj-dark-blue', bg: '--wj-dark-blue', why: `lifted blue ${l.toFixed(0)}%` };
  if (l >= 55) return { fg: '--wj-muted', bg: '--wj-muted', why: `muted ${l.toFixed(0)}%` };
  if (l >= 42 && s > 45) return { fg: '--wj-blue', bg: '--wj-blue', why: `brand blue ${l.toFixed(0)}%` };
  if (l >= 38) return { fg: '--wj-muted', bg: '--wj-muted', why: `muted ${l.toFixed(0)}%` };
  if (l >= 30 && s > 45) return { fg: '--wj-blue-press', bg: '--wj-blue-press', why: `pressed blue ${l.toFixed(0)}%` };
...
for (const [literal, n] of [...counts.entries()].sort((a, b) => b[1] - a[1])) {
  ...
  map[literal] = {
    uses: n,
    fg: pinned ? pinned.fg : auto.fg,
    bg: pinned ? pinned.bg : auto.bg,
    source: pinned ? 'pinned' : 'classified',
...
console.log(`literals: ${Object.keys(map).length} (${pinnedCount} pinned, ${Object.keys(map).length - pinnedCount} classified)`);
